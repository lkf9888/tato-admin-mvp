...
**Each language has its own address.** The site used to pick a language
from a cookie and the browser's Accept-Language. Google's crawler sends
neither, so it only ever indexed English, and a renter searching in
Chinese could not land on a Chinese page. Now English is at the root,
Simplified Chinese at `/zh-CN` and Traditional at `/zh-TW` — the scheme
the operator's company site already uses — on a custom domain and under
`/s/<slug>` alike. Every page carries hreflang for its siblings and
`x-default`, the sitemap lists all three with alternates, `<html lang>`
follows the URL, and the language menu is plain links a crawler can
follow. Stripe Checkout opens in the renter's language and sends them
back to the page they were reading.

**The operator's own words are translatable.** Site name, eyebrow,
headline, introduction, opening hours, footer and the headline numbers'
labels each have English, 简体 and 繁體 fields, edited on three tabs of
one form. Each field falls back on its own — a translated headline over
an untranslated footer is a Chinese headline and an English footer, not
an all-English page — and an empty Traditional field is the Simplified
one converted, before English. New fields for opening hours, a WeChat
ID, and up to four headline numbers.

**Traditional Chinese was wrong in ways nobody could see from Simplified.**
It was produced at runtime by a 250-character substitution table. An
OpenCC audit of all 1,491 strings found 89 common characters missing —
预订 rendered as 預订 — 52 characters converted in the wrong sense
(公里 became 公裡, 日历 became 日歷), and every message *function* passed
through untouched, so any string with a number or a name in it rendered
in Simplified. The Traditional messages are now generated by running
OpenCC (Taiwan standard and phrasing) over the source of every `zh`
block, which converts the text inside functions too. The output is
committed, reviewed as a diff, and CI fails if it is stale
(`npm run i18n:zh-hant`). Status and activity labels moved into a
messages module so they are generated with everything else. A short
override list fixes where OpenCC's phrasing picks the wrong sense here
(审核通过 is 審核通過, not 稽覈透過; 帳 and 台 as Taiwan writes them).

An operator's own text is converted characters-only. Vocabulary
substitution segments text it has never seen and invents words:
"列治文本地团队" (a Richmond local team) was read as 列治 + 文本 and came
out 列治文字地團隊.

**The look follows the operator's company site.** Inter, near-black
ink, pill buttons, 24px cards, a hero lit by two soft glows, and the
dark numbers card. Every tint is mixed from the site's accent colour,
so another operator's colour re-tints the whole site. New sections:
how booking works in three steps, and a contact block with address,
hours, phone, email, WeChat and a map link. Scoped to the public site;
the admin is unchanged.

Also fixed on the way: social-preview and structured-data image URLs on
`/s/<slug>` sites pointed at `/s/<slug>/api/…`, which 404s; the renter
saw "This prototype checkout flow is ready for the next step" after
paying; a car page with no intro told Google visitors they need not "go
back to Turo".
