      ...
      <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl overflow-hidden rounded-[2rem] border border-white/70 bg-white shadow-xl lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <section className="flex items-center px-6 py-10 sm:px-10">
          <div className="mx-auto w-full max-w-md">
            <p className="text-xs uppercase tracking-[0.35em] text-slate-500">{registerMessages.kicker}</p>
            <h2 className="mt-3 font-serif text-4xl text-slate-950">{registerMessages.title}</h2>
            <p className="mt-3 text-sm text-slate-600">{registerMessages.description}</p>
            ...
            <div className="mt-6 rounded-3xl border border-slate-200 bg-white px-5 py-4 text-sm text-slate-600">
              <p className="font-medium text-slate-900">{registerMessages.loginPrompt}</p>
              <a href="/login" className="mt-2 inline-flex text-sm font-medium text-slate-950 underline underline-offset-4">
