...
import { FileText, FileUp, ImageUp } from "lucide-react";
...
function getAttachmentUrl(attachment: CalendarAttachment) {
  return `/api/order-attachments/${attachment.id}/file`;
}
...
export function CalendarView({
  ...
  const [uploadingAttachmentCategory, setUploadingAttachmentCategory] = useState<"media" | "contract" | null>(null);
  ...
  const handleAttachmentUpload = async (
    ...
    if (files.length === 0 || uploadingAttachmentCategory) return;
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as
        | { attachments?: CalendarAttachment[]; error?: string }
        | null;

      const uploadedAttachments = payload?.attachments;

      if (!response.ok || !Array.isArray(uploadedAttachments)) {
        setAttachmentActionError(calendarMessages.attachmentUploadError);
        return;
      }
  ...
  const renderAttachmentArea = (order: CalendarOrder, compact = false) => {
    if (readOnly) return null;

    const attachments = order.attachments ?? [];
    const mediaAttachments = attachments.filter((attachment) => attachment.category === "media");
    const contractAttachments = attachments.filter((attachment) => attachment.category === "contract");
    const isUploadingMedia = uploadingAttachmentCategory === "media";
    const isUploadingContract = uploadingAttachmentCategory === "contract";

    return (
      <div className={cn("space-y-3", compact ? "mt-4" : "mt-2 lg:col-span-2")}>
        <div className="flex flex-wrap gap-2">
          <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-semibold text-slate-700 transition hover:border-slate-900 hover:text-slate-950">
            <ImageUp className="h-4 w-4" />
            {isUploadingMedia ? calendarMessages.uploadingMedia : calendarMessages.uploadMedia}
            <input
              type="file"
              accept="image/*,video/*"
              multiple
              className="hidden"
              disabled={Boolean(uploadingAttachmentCategory)}
              onChange={(event) => {
                void handleAttachmentUpload(order, "media", event.currentTarget.files);
          ...
          <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-[12px] font-semibold text-slate-700 transition hover:border-slate-900 hover:text-slate-950">
            ...
            {isUploadingContract ? calendarMessages.uploadingContract : calendarMessages.uploadContract}
            <input
              type="file"
              multiple
              className="hidden"
              disabled={Boolean(uploadingAttachmentCategory)}
              onChange={(event) => {
                void handleAttachmentUpload(order, "contract", event.currentTarget.files);
                event.currentTarget.value = "";
              }}
            />
          </label>
        ...
        <div className={cn("grid gap-3", compact ? "" : "lg:grid-cols-2")}>
          <div className="rounded-2xl bg-slate-50 p-3">
            ...
            {mediaAttachments.length > 0 ? (
                ...
                {mediaAttachments.map((attachment) => {
                  ...
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="overflow-hidden rounded-xl border border-slate-200 bg-white text-[11px] text-slate-600"
                    >
                      {isImage ? (
                        <img src={url} alt={attachment.originalName} className="h-20 w-full object-cover" />
                      ) : isVideo ? (
              ...
              <p className="mt-2 text-[12px] text-slate-400">{calendarMessages.noMediaFiles}</p>
            )}
          </div>

          <div className="rounded-2xl bg-slate-50 p-3">
            <p className="text-[11px] font-semibold text-slate-600">
              {calendarMessages.contractSection}
            </p>
            {contractAttachments.length > 0 ? (
              <div className="mt-2 space-y-2">
                {contractAttachments.map((attachment) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[12px] text-slate-700 transition hover:border-slate-900 hover:text-slate-950"
                  >
              ...
              <p className="mt-2 text-[12px] text-slate-400">{calendarMessages.noContractFiles}</p>
            )}
          </div>
        </div>
      </div>
    ...
    <div className="space-y-3">
      ...
      {selectedOrder && orderPopover ? (
        ...
        >
          ...
          {renderAttachmentArea(selectedOrder, true)}
      ...
      <section className="rounded-2xl border border-white/70 bg-white/90 p-3 shadow-sm">
        ...
        {selectedOrder ? (
          <div className="mt-2.5 grid gap-3 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            {renderAttachmentArea(selectedOrder)}
