...
function labels(locale: Locale) {
  return locale === "zh"
    ? {
        ...
        uploading: "上传中...",
        loading: "读取附件中...",
        emptyPhotos: "还没有车辆照片或视频。",
        emptyDocs: "还没有车辆文件。",
        delete: "删除",
        deleteConfirm: "确定隐藏这个车辆附件吗？记录会保留在后台。",
        error: "车辆附件暂时无法处理，请稍后再试。",
      }
    : {
        title: "Vehicle photos, videos, and files",
        ...
        emptyPhotos: "No vehicle photos or videos yet.",
        emptyDocs: "No vehicle files yet.",
        delete: "Delete",
        deleteConfirm: "Hide this vehicle attachment? The backend record will be preserved.",
        error: "We could not process vehicle attachments right now. Please try again.",
      };
}

function isVideo(attachment: VehicleAttachment) {
  const type = attachment.contentType?.toLowerCase() ?? "";
  const name = attachment.filename?.toLowerCase() ?? "";
  return type.startsWith("video/") || /\.(mp4|mov|m4v|webm|3gp|avi|qt)$/.test(name);
}

function isImage(attachment: VehicleAttachment) {
  const type = attachment.contentType?.toLowerCase() ?? "";
  const name = attachment.filename?.toLowerCase() ?? "";
...
export function VehicleAttachments({
  vehicleId,
  locale,
  compact = false,
}: {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch(`/api/vehicles/${vehicleId}/attachments`)
  ...
  async function deleteAttachment(attachment: VehicleAttachment) {
    if (!window.confirm(copy.deleteConfirm)) return;

    const response = await fetch(`/api/vehicles/${vehicleId}/attachments/${attachment.id}`, {
      method: "DELETE",
    });
    if (!response.ok) {
      setError(copy.error);
...
function AttachmentGroup({
  ...
  locale: Locale;
  deleteLabel: string;
  onDelete: (attachment: VehicleAttachment) => void;
}) {
  return (
