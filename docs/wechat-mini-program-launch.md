...
**已定：个人主体。** 对照表留着，因为这个决定一年后会有人问「当初为什么」。
...
| 订阅消息 | 能发 | 能发 |
| `web-view` | 可以用 | **不能用** |
| 业务域名 | 要配（多一步校验文件） | 配不了，也不用配 |

实际损失只有一处：**通知点不进各系统自己的网页**。五个页面里只有 `pages/webview`
失效，已经从 `app.json` 摘掉：

| 页面 | 个人主体下 |
| --- | --- |
| `pages/message` 消息列表 + 详情 | 正常 |
| `pages/bind` 绑定码加频道 | 正常 |
| `pages/tasks` 任务列表、完成任务、**传照片** | 正常（原生 `wx.uploadFile`） |
| `pages/login` | 正常 |
| `pages/webview` | 已停用 |

所以 TATO 的通知显示「查看任务」，直接跳原生任务页——员工照样完成任务、传照片，
只是不再一键落到 `/staff-share`。**HostHub 和洗车系统没有原生页面兜底**，它们的
通知只会显示「复制链接」。这是这个决定真正的代价，接那两个系统前要想起来。

将来改企业主体：把 `pages/webview` 加回 `app.json`，`pages/message/index.js` 的
`resolveAction` 加一条 `kind: "web"` 分支。

- [x] 确认用哪个主体注册 —— 个人主体 <!-- id:s0a -->
...
- [ ] 注册小程序，记下 AppID（形如 `wx1a2b3c4d5e6f7890`） <!-- id:s1a -->
      ...
      个人主体的名称不能用企业名或品牌词，「TATO」可能被判成品牌名——备一个
      替代名，比如「车队任务助手」。类目选工具类。
...
- [x] ~~配业务域名~~ —— **个人主体跳过这步**，配不了也不需要。 <!-- id:s1d -->
      改成企业主体才要：加 `tatocar.co`，下载校验文件传到服务器根目录再校验
